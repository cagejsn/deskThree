...
import UIKit
import AVFoundation
class PlayerVC: UIViewController, UITableViewDelegate, UITableViewDataSource, UIGestureRecognizerDelegate {
    ...
    func customization() {
        ...
        self.player.addGestureRecognizer(UITapGestureRecognizer.init(target: self, action: #selector(PlayerVC.tapPlayView)))
        NotificationCenter.default.addObserver(self, selector: #selector(PlayerVC.tapPlayView), name: NSNotification.Name("open"), object: nil)
    }
