...
public class Zip {
    ...
    public class func unzipFile(_ zipFilePath: URL, destination: URL, overwrite: Bool, password: String?, progress: ((_ progress: Double) -> ())?) throws {
        ...
        let fileFound = fileManager.fileExists(atPath: path) == false
        let extensionValid = fileExtensionIsInvalid(zipFilePath.pathExtension)
        
        if fileFound || extensionValid {
            
            print("file not found")
            print(fileFound)
            print(extensionValid)
